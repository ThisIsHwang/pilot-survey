...
MARKER = "# STACKPILOT_EXPERIMENT_ENV_V1"
...
def patch(search_r1_root: Path) -> None:
    ...
    if count != 1:
        raise RuntimeError(f"Expected one pinned ray.init line in {target}; found {count}")
    target.write_text(text.replace(OLD_RAY_INIT, NEW_RAY_INIT, 1), encoding="utf-8")
