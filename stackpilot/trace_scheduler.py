...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run TRACE LoRA jobs across one H100 node.")
    ...
    if args.experiment:
        ...
        jobs = [job for job in jobs if job["experiment_id"] in selected]
    if not args.force:
    ...
    def request_stop(signum: int, _frame: Any) -> None:
        ...
        interrupted = True
        print(f"TRACE scheduler received signal {signum}; stopping workers.", flush=True)
    ...
    if interrupted:
        raise SystemExit("TRACE scheduler interrupted; unfinished jobs remain resumable")
    ...
    if failures:
        raise SystemExit(f"{len(failures)} TRACE jobs failed; see {summary_path}")
    print(f"TRACE scheduler completed {len(jobs)} jobs")
