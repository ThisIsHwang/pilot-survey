...
def _candidate_bank(
    ...
    *,
    ...
    """Sample search actions from the served policy at one frozen state.

    The source artifact may have been created by an older policy. Its factual
    query is therefore excluded by default and is never mislabeled as a direct
    Qwen3.5 sample. Controlled source candidates are also disabled in the main
    profiles. This makes the action comparison conditional on frozen states but
    fully native to the served Qwen3.5 policy at the action boundary.
    """
    ...
    if bool(profile.get("include_source_factual", False)):
        add(str(state["factual_query"]), "source-policy-factual")

    generation = profile["sibling_generation"]
    for attempt in range(int(generation["attempts"])):
        if len(candidates) >= target:
            break
        generation_seed = stable_seed(
            "weekend-qwen35-direct-action", state["state_id"], attempt
        )
        content = _complete(
            causal_cfg,
    ...
    if len(candidates) < target and bool(profile.get("allow_controlled_fallback", False)):
        for row in result.get("candidates", []):
            if len(candidates) >= target:
                break
            if int(row.get("protocol_failure", 0)) != 0:
                continue
            add(str(row.get("query", "")), "controlled-source-policy-fallback")

    if len(candidates) < minimum:
        raise RuntimeError(
            f"State {state['state_id']} has only {len(candidates)} direct served-policy search actions"
        )
