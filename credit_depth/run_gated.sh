...
METHODS=${METHODS:-"outcome verifiable necessity-gated reliability-gated creditability-gated"}
...
for backend in $BACKENDS; do
  ...
  for method in $METHODS; do
    for seed in $SEEDS; do
      EXPERIMENT_ID=EXP-066 PROFILE="$PROFILE" BACKEND="$backend" METHOD="$method" SEED="$seed" \
